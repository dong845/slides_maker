...
| `SLIVER_GAP` • | Two blocks almost touch (a hair-thin gap) — reads as a rendering accident | Snap them flush (share an edge) or open the gap to ≥0.12 in |
...
- **Usually act:** `INVERTED TYPE HIERARCHY`, `TIMID COVER`, `SMALL TYPE` on a presented deck,
